...
class articulos extends \core\Controlador{
    ...
    public function expansiones(array $datos = array()){
        $object = new \modelos\articulos();
        $datos['filas'] = $object->getExpansiones();
        
        $this->index($datos);
        
    }
    public function categoria(array $datos = array()){
        $request = \core\HTTP_Requerimiento::request();
        if(isset($request['p3'])){
            $datos['filas'] = \modelos\articulos::getJuegos_by_categoria($request['p3']);
        }
